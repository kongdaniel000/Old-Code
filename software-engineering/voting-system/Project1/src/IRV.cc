...
Candidate* IRV::maxCandWinCheck(vector<bool> inRunning) {
    ...
    int totalVotes = 0;
    for (int i = 0; i < numCandidates; i++) {                          // to find total votes, only tally for non eliminated candidates
        if (inRunning[i]) {           
...
void IRV::printResults(Candidate winner, int redists) {   // check working votes situation
    ...
    for (int i = 0; i < numCandidates; i++) {
        ...
        for (int j = 0; j < redists; j++) {                                                                // loop to print out each vote redistribution for a candidate
            s.str("");
            if (candidates[i].getGainedVotes()[j] == -1) {                                                   // if the candidate was/is eliminated, they did not gain any votes
                cout << left << setw(13) << "----------";
                cout << left << setw(13) << "----------";
            }
            else {
